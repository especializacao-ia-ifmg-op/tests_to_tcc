...
print(f'[uni_lat-21.95_lon-46.65_CNN3.py]\n')
print(f'Getting dataset...')
u_row_train, u_row_test = get_search_dataset('uni_lat-21.95_lon-46.65.csv')
print(f'...OK!\n')

u_results_CNN3 = []
u_train, u_test, u_scaler = es.get_dados(star_CNN3, u_row_train, u_row_test)
X_train, y_train, X_test, y_test = basic.slideWindow(u_train, u_test, star_CNN3['lags'])
start = time.time()
for i in range(5):
    model,_ = basic.modelo_CNN3(X_train, y_train, star_CNN3)
    rmse, yhat, y_test = basic.predictModel(u_test, model, 10, star_CNN3['lags'], scaler=u_scaler)
    u_results_CNN3.append(rmse)
    print(f'{i+1}:')
    
stop = time.time()
u_results_CNN3 = form_data(u_results_CNN3, '3 (ETo)')
u_results_CNN3.to_csv('u_results_CNN3.csv',index=True)
